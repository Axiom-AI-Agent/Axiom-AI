...
## Corpus Check
- 325 files · ~459,403 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2859 nodes · 5371 edges · 231 communities (194 shown, 37 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 909 edges (avg confidence: 0.69)
...
Cohesion: 0.05
Nodes (82): get_drive_tool(), get_rag_tool(), get_request_id(), Request, FastAPI dependency injection helpers., _require_startup(), HTTP chat endpoints — WhatsApp-like dev interface (no Twilio required)., active_config() (+74 more)
...
Cohesion: 0.30
Nodes (7): main(), main(), ChatPipeline, Channel-agnostic chat pipeline — HTTP dev chat + Twilio webhook., Sync entry for scripts and tests without a running event loop., InboundMessage, Normalized inbound message from any channel.
...
Cohesion: 0.21
Nodes (11): _build_user_prompt(), get_current_trace_id(), langfuse_turn_attributes(), Any, OpenTelemetry / Langfuse trace id for the active context, if any., Propagate user/session/tags to nested spans for one chat turn., Attach I/O + usage to the current span/generation., _tracing_enabled() (+3 more)
...
Cohesion: 0.18
Nodes (18): Enrollment, Base, Represents a student's enrollment in a subject class., create_student(), delete_student(), enroll_student(), get_student(), get_student_by_phone() (+10 more)
...
Cohesion: 0.25
Nodes (10): _parse_form_params(), post, Request, Response, Twilio WhatsApp webhook router., Twilio WhatsApp sandbox webhook.      Returns 200 immediately and processes the, _should_validate_signature(), twilio_webhook() (+2 more)
...
Cohesion: 0.22
Nodes (10): get_chat_model(), get_embedding_model(), _get_nested(), get_role_config(), _get_str(), _load_yaml(), Any, Application configuration — YAML + environment variables.  Secrets live only in (+2 more)
...
Cohesion: 0.14
Nodes (28): ChatChannel, EnrollmentStatus, EscalationStatus, FeeCycle, InvoiceStatus, MessageRole, PaymentStatus, Enum (+20 more)
...
Cohesion: 0.18
Nodes (13): _body_for_pipeline(), handle_text_message(), client(), asyncio, fixture, Telegram webhook + ChatPipeline wiring tests., test_handle_contact_greets_already_enrolled_student(), test_handle_contact_starts_enrollment_for_new_phone() (+5 more)
...
Cohesion: 0.29
Nodes (12): _dispatch(), Any, _display_name(), handle_contact_shared(), handle_photo_message(), handle_voice_message(), _largest_photo_file_id(), Any (+4 more)
...
Cohesion: 0.18
Nodes (13): _chain_mock(), main(), post, Staff reply to a student via WhatsApp.      Persists the message as role=system, send_staff_message(), assert_body_tenant(), assert_session_for_tenant(), Tenant scope validation for staff / dashboard endpoints. (+5 more)
...
- **Why does `IdentityContext` connect `Test Suite 3` to `Dashboard Module 4`, `Chat Message Pipeline`, `Smoke Scripts`, `Chat Message Pipeline 2`, `Chat Message Pipeline 3`, `Admissions Onboarding 15`, `Escalation Routes 6`, `Drive Tooling 4`, `Dashboard API Routes 2`, `Agent Orchestrator`, `Drive Tooling 3`, `Admissions Onboarding 9`?**
...
- **Why does `DriveTool` connect `Admissions Onboarding 7` to `Demo Chat Lifecycle 2`, `Memory MCP Path`, `Admissions Onboarding 5`, `Resource Agent 2`, `Student Chat Deps 4`?**
