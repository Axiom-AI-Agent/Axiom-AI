# Graph Report - Axiom-AI  (2026-08-08)

## Corpus Check
- 308 files · ~447,275 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2654 nodes · 4918 edges · 214 communities (182 shown, 32 thin omitted)
- Extraction: 84% EXTRACTED · 16% INFERRED · 0% AMBIGUOUS · INFERRED: 788 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `46068707`
...
5. `CrmTool` - 38 edges
6. `ChatChannel` - 38 edges
7. `RagTool` - 36 edges
8. `DriveTool` - 32 edges
9. `EnrollmentStatus` - 31 edges
10. `EscalationStatus` - 31 edges
...
Cohesion: 0.06
Nodes (30): OnboardingFlow, OnboardingSlots, OnboardingState, Any, Multi-turn admissions onboarding — slot tracking and class disambiguation., Determine onboarding progress and extract slots from user messages., Hydrate state for an existing database student (post-enrollment paths only)., OnboardingSession (+22 more)

### Community 1 - "Admissions Onboarding 2"
Cohesion: 0.06
Nodes (47): main(), AdmissionsAgent, AdmissionsAgentResult, _last_user_text(), Any, Admissions agent node — multi-turn onboarding via CRM MCP tools., classify_info_inquiry(), extract_class_filters() (+39 more)

### Community 2 - "Demo Chat Lifecycle"
Cohesion: 0.06
Nodes (27): CrmClient, DirectCrmClient, McpCrmClient, Any, Protocol, Shared CRM client protocol for agent nodes (direct + MCP paths)., MCP CRM tools → async dispatch., In-process CRM path (dev/tests without MCP subprocesses). (+19 more)

### Community 3 - "RAG Ingest Pipeline"
Cohesion: 0.20
Nodes (19): qdrant_collection_for_tenant(), collection_exists(), collection_info(), count_points(), delete_collection(), ensure_class_id_index(), ensure_collection(), get_qdrant_client() (+11 more)
...
Cohesion: 0.07
Nodes (41): main(), build_orchestrator(), In-process MemoryTool path (dev/tests without MCP subprocesses)., get_api_key(), get_chat_model(), get_embedding_model(), _get_nested(), get_role_config() (+33 more)

### Community 6 - "Dashboard Frontend Pages"
Cohesion: 0.07
Nodes (37): ClassFormState, emptyForm, ApiError, assignEscalation(), ChatConversation, ChatSender, ChatThread, ChatTurn (+29 more)
...
Cohesion: 0.14
Nodes (19): Agent framework — decision graph + orchestrator (Phase 2)., run_admissions_agent(), AgentOrchestrator, _emit_from_config(), _format_session_memory(), _invoke_llm_text(), _last_user_text(), _llm_content_to_str() (+11 more)
...
Cohesion: 0.25
Nodes (30): ChatConversationsResponse, ChatConversationSummary, ChatRequest, ChatResponse, ChatThreadResponse, ChatTurnRecord, ChatTurnsResponse, ConfigResponse (+22 more)
...
Cohesion: 0.12
Nodes (15): main(), Same business logic memory_server exposes — valid when Python < 3.10., _run_mcp_adapter_path(), _run_memory_tool_fallback(), _seed_memory(), _mcp_text(), McpDriveClient, McpRagClient (+7 more)
...
Cohesion: 0.11
Nodes (14): CrmTool, Open (or return existing) escalation for dashboard inbox., Legacy alias — creates payment_receipt escalation without bank_slip storage., Update student profile fields for onboarding., Atomic post-confirmation write: student profile + pending enrollment., mock_db(), fixture, CRM tool and tenant isolation tests. (+6 more)
...
Cohesion: 0.11
Nodes (17): Any, build_drive_backend(), DriveBackend, GoogleDriveBackend, MockDriveBackend, _prefer_ipv4_for_urllib3(), Any, Protocol (+9 more)
...
Cohesion: 0.15
Nodes (19): ClassesPage(), IngestContent(), MessagesContent(), senderBubbleClass(), profileToForm(), SettingsFormState, SettingsPage(), TenantSelector() (+11 more)
...
Cohesion: 0.16
Nodes (13): build_decision_graph(), _emit_from_config(), make_guardrail_node(), make_router_node(), EmitFn, RunnableConfig, Decision LangGraph — guardrail and router subgraph for Axiom AI.  Ported from Bo, _build_user_prompt() (+5 more)

### Community 32 - "Admissions Onboarding 10"
Cohesion: 0.14
Nodes (20): ChatResult, _noop_emit(), Any, EmitFn, Single async entry for one chat turn: decision graph → orchestrator (or OOS shor, _routes_from_patch(), run_chat_turn(), flush() (+12 more)
...
Cohesion: 0.13
Nodes (20): get_db(), get_tenant_id(), create_message_log(), get_message_logs(), get, post, Session, approve_payment() (+12 more)

### Community 36 - "Design Double Diamond 4"
Cohesion: 0.14
Nodes (18): InboxContent(), isPaymentReason(), statusClass(), EscalationSocketEvent, EscalationSocketEventType, useEscalationSocket(), UseEscalationSocketOptions, usePolling() (+10 more)
...
Cohesion: 0.19
Nodes (19): assign_escalation(), create_new_escalation(), get_escalations(), get_open_escalations(), get_tenant_escalation_or_404(), Escalation, get, post (+11 more)

### Community 39 - "Chat Message Pipeline 2"
Cohesion: 0.13
Nodes (17): DriveTool, _normalize_drive_folder_id(), Drive tool — tenant-scoped paper/textbook/syllabus search., Strip URL query junk users paste from Drive share links (e.g. ``?usp=drive_link`, Business logic for drive_search / drive_list — used by drive_server and REST., Drive MCP server — tool surface and tenant scoping (same logic as axiom-drive st, test_drive_mcp_list_folder(), test_drive_mcp_rejects_disallowed_folder() (+9 more)
...
Cohesion: 0.10
Nodes (34): get_current_staff(), Session, Authenticated tenant scope for dashboard API requests., Base, Represents a staff member of a tuition institute., StaffUser, bootstrap_demo_physics(), login() (+26 more)
...
Cohesion: 0.17
Nodes (14): main(), main(), _one_turn(), get_chat_turns(), ChatTurnsResponse, get, Fetch recent conversation turns for a student session., session_id_for_phone() (+6 more)
...
Cohesion: 0.13
Nodes (11): CRM business logic — called by MCP server only (Week 13 pattern)., Escalation reason codes for dashboard inbox filtering., Supabase access for admissions CRM operations., mock_db(), fixture, CRM escalation create/resolve tests for Phase 5 flows., test_create_payment_escalation(), test_create_talk_to_tutor_escalation() (+3 more)
...
Cohesion: 0.16
Nodes (13): fixed_chunk(), parent_child_chunk(), Any, Text chunking strategies — fixed + parent-child (Week 13 pattern)., Split documents into fixed-size chunks with overlap., Two-tier chunking: small child chunks indexed in Qdrant, parent text stored, patch, Ingest pipeline unit tests. (+5 more)

### Community 61 - "Workshop Exploration 2"
Cohesion: 0.24
Nodes (14): _build_parent_lookup(), embed_texts(), _enrich_children_with_parent_text(), ingest_documents(), load_tenant_docs(), Any, Tenant-scoped tutor-note ingestion into Qdrant., Chunk, embed, and upsert documents into the tenant Qdrant collection.      Paren (+6 more)
...
Cohesion: 0.30
Nodes (10): ResourceAgent, FakeDrive, FakeMemoryTool, FakeRag, asyncio, Identity recall and resource enrollment gate tests., test_build_recall_context_includes_profile_before_st(), test_resource_agent_allows_pending_enrollment() (+2 more)
...
Cohesion: 0.19
Nodes (15): _disable_langfuse(), get_langfuse_client(), _is_langfuse_auth_error(), is_langfuse_enabled(), langfuse_disabled_reason(), prefetch_prompts(), Validate API keys once at startup; avoids repeated 401 noise from prompt fetch., Return Langfuse client or None when disabled / unconfigured. (+7 more)

### Community 70 - "MCP Integration"
Cohesion: 0.12
Nodes (12): get, FastAPI application — Phase 6 integration (MCP warmup + dashboard APIs)., root(), Request, Response, Request ID and latency headers., RequestContextMiddleware, _InterceptHandler (+4 more)
...
Cohesion: 0.09
Nodes (21): classify_resource_subpath(), DirectDriveClient, DirectRagClient, DriveClient, _infer_drive_folder(), _last_user_text(), Protocol, RagClient (+13 more)

### Community 86 - "Dashboard Module 3"
Cohesion: 0.20
Nodes (8): _mask_path(), step1_environment(), main(), main(), TEMP DEBUG — Google OAuth only (no Drive list, no MCP).  Run:   PYTHONPATH=src ., _load_env(), Load env from Dashboard, shared AI-backend, or repo root., Path
...
Cohesion: 0.09
Nodes (21): get_drive_tool(), get_rag_tool(), get_request_id(), Request, FastAPI dependency injection helpers., _require_startup(), HTTP chat endpoints — WhatsApp-like dev interface (no Twilio required)., list_files() (+13 more)
...
Cohesion: 0.29
Nodes (7): build_recall_context(), format_student_profile(), Build recall context (student profile + ST turns) before the decision graph., Structured student block for router and agent prompts., Return (full_router_context, student_profile_context) for one chat turn., test_format_student_profile_enrolled(), test_format_student_profile_unknown_visitor()
...
## Knowledge Gaps
- **562 isolated node(s):** `name`, `private`, `version`, `type`, `description` (+557 more)
...
- **Why does `CrmTool` connect `Decision Graph` to `Demo Chat Lifecycle`, `MCP Integration 3`, `Admissions Onboarding 18`, `Admissions Onboarding 6`, `Drive Tooling`, `Workshop Exploration`, `Langfuse Runtime 3`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `AgentState` connect `Admissions Onboarding 4` to `Admissions Onboarding 2`, `Demo Chat Lifecycle`, `Test Suite 3`, `IdentityResolver`, `API Chat Schemas`, `Drive Tooling 4`, `Resource Agent 2`, `Admissions Onboarding 11`, `Dashboard API Routes 2`, `Test Suite 2`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `EscalationActionResponse` connect `Workshop Exploration` to `Admissions Onboarding`, `Smoke Scripts`, `Invoice DB Layer 2`, `Admissions Onboarding 15`, `Decision Graph`, `Drive Tooling 3`?**
