...
## Corpus Check
- 371 files · ~492,117 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3530 nodes · 6901 edges · 242 communities (207 shown, 35 thin omitted)
- Extraction: 82% EXTRACTED · 18% INFERRED · 0% AMBIGUOUS · INFERRED: 1226 edges (avg confidence: 0.7)
...
Cohesion: 0.06
Nodes (39): _format_lkr_amount(), OnboardingFlow, OnboardingSlots, OnboardingState, Any, Multi-turn admissions onboarding — slot tracking and class disambiguation., Determine onboarding progress and extract slots from user messages., Hydrate state for an existing database student (post-enrollment paths only). (+31 more)

### Community 1 - "Admissions Onboarding 2"
Cohesion: 0.07
Nodes (51): main(), AdmissionsAgent, AdmissionsAgentResult, _last_user_text(), Any, Admissions agent node — multi-turn onboarding via CRM MCP tools., run_admissions_agent(), classify_info_inquiry() (+43 more)
...
Cohesion: 0.05
Nodes (54): main(), build_orchestrator(), In-process MemoryTool path (dev/tests without MCP subprocesses)., analyze_tenant_faqs(), Any, DashboardTenant, FAQ intelligence endpoint for dashboard staff., Cluster recent student questions into recurring FAQ themes. (+46 more)
...
Cohesion: 0.14
Nodes (14): get_tenant_id(), Authenticated tenant scope for dashboard API requests., get, root(), escalation_websocket(), websocket, get_tenant_profile(), list_tenants() (+6 more)
...
Cohesion: 0.06
Nodes (62): _ingest_result(), _minimal_docx(), _post(), patch, Multi-format ingest upload endpoint tests., test_delete_ingest_document(), test_get_ingest_document(), test_ingest_upload_docx() (+54 more)

### Community 43 - "Decision Graph 2"
Cohesion: 0.12
Nodes (27): _fetch_open_escalations_by_student(), _fetch_open_escalations_for_student(), _fetch_students_by_ids(), get_chat_turns(), get_conversation_thread(), get_thread_alias(), build_conversation_summary(), phone_from_session_id() (+19 more)
...
Cohesion: 0.16
Nodes (20): _build_user_prompt(), build_guardrail_system_prompt(), build_resource_drive_list_reply(), build_resource_drive_pick_reply(), build_resource_drive_reply(), build_resource_rag_reply(), _drive_folder_label(), get_admissions_stub_reply() (+12 more)
...
Cohesion: 0.10
Nodes (20): Dashboard API Contract (Phase 5), Document ingest (knowledge base), Endpoint map, Get thread (message panel), How to pass tenant, Integration flow (recommended), Legacy chat logs, List conversations (sidebar) (+12 more)

### Community 50 - "Langfuse Runtime 2"
Cohesion: 0.08
Nodes (30): main(), main(), _one_turn(), get_chat_turns(), ChatTurnsResponse, get, Fetch recent conversation turns for a student session., session_id_for_phone() (+22 more)
...
Cohesion: 0.26
Nodes (12): _latin_tokens(), looks_like_singlish(), looks_like_tanglish(), Reply-language resolution for Sinhala, Tamil, English, and code-switching.  Nati, True for romanized Sinhala mix (Singlish), not native Sinhala script., True for romanized Tamil mix (Tanglish), not native Tamil script., Locale for canned (non-LLM) strings, including latin Singlish/Tanglish., English-biased search string for tutor notes; keep the original as the LLM quest (+4 more)
...
Cohesion: 0.12
Nodes (26): main(), title_from_filename(), _attach_document_ids(), delete_document_ingest(), embed_texts(), ingest_documents(), load_tenant_docs(), prepare_upload_ingest() (+18 more)
...
Cohesion: 0.10
Nodes (31): EscalationAgent, EscalationAgentResult, _last_user_text(), Escalation agent — talk-to-tutor requests → dashboard inbox., run_escalation_agent(), DirectDriveClient, McpDriveClient, McpRagClient (+23 more)
...
Cohesion: 0.18
Nodes (19): Enrollment, Base, Represents a student's enrollment in a subject class., create_student(), delete_student(), enroll_student(), get_student(), get_student_by_phone() (+11 more)
...
Cohesion: 0.14
Nodes (19): _langfuse_template(), main(), Convert Python .format `{var}` placeholders to Langfuse `{{var}}`., _seed_catalog(), _disable_langfuse(), get_langfuse_client(), _is_langfuse_auth_error(), is_langfuse_enabled() (+11 more)
...
Cohesion: 0.12
Nodes (19): build_decision_graph(), make_guardrail_node(), make_router_node(), Decision LangGraph — guardrail and router subgraph for Axiom AI.  Ported from Bo, get_guardrail(), Guardrail, Any, Domain Guardrail — tuition scope filter for the decision subgraph.  Ported from (+11 more)

### Community 85 - "Resource Agent 2"
Cohesion: 0.19
Nodes (17): decide_node(), Any, get_out_of_scope_reply(), MultiRouteDecision, RouteDecision, asyncio, Decision graph routing tests — ported from BookMe scripts/test_decision_graph.py, test_decide_out_of_scope_short_circuit() (+9 more)
...
Cohesion: 0.16
Nodes (12): build_escalation_ack_reply(), build_payment_ack_reply(), build_payment_missing_media_reply(), canned_language_parent(), normalize_language_pref(), si_latn → si, ta_latn → ta, otherwise the normalized pref., Map stored / incoming codes to en | si | ta. Unknown values become en., Canned WhatsApp strings for en / si / ta. LLM-generated replies use mirroring in (+4 more)
...
Cohesion: 0.17
Nodes (15): detect_script_language(), Return si/ta when native script is present; None for Latin-only text., Script of this message wins; otherwise stored preference; otherwise English., resolve_reply_language(), parametrize, Language detection, mirroring policy, routing, and canned-string eval., test_detect_script_latin_is_none(), test_detect_script_sinhala() (+7 more)
...
Cohesion: 0.67
Nodes (3): Human-readable hint for STT. None for English so Gemini auto-detects., stt_language_hint(), test_stt_hint_skips_english()
...
Cohesion: 0.29
Nodes (4): build_router_prompt(), _fallback_multi(), Any, QueryRouter
...
Cohesion: 0.20
Nodes (11): main(), _primary_route(), RoutingCase, _run(), map_decision_to_agent_state(), Bridge decision subgraph output → orchestrator AgentState.  Ported from BookMe A, build_decision_input(), DecisionState (+3 more)
...
Cohesion: 0.13
Nodes (8): CRM business logic — called by MCP server only (Week 13 pattern)., Reason-aware resolve: payment → activate enrollment; tutor → close only., Staff approves payment — activates pending enrollment., Staff rejects payment — closes escalation without activating enrollment., Backward-compatible alias for payment resolve., is_payment_reason(), Escalation reason codes for dashboard inbox filtering., Supabase access for admissions CRM operations.
...
Cohesion: 0.40
Nodes (5): Escalation inbox (unified HITL queue), Escalation object, Query params (list), Reject payment, Resolve (approve payment / close tutor)
...
Cohesion: 0.67
Nodes (3): _emit_from_config(), EmitFn, RunnableConfig
...
- **Why does `ChatChannel` connect `Invoice DB Layer 2` to `Test Suite 3`, `Escalation Routes 4`, `Dashboard Module`, `test_identity_recall.py`, `Escalation Routes 8`, `Workshop Exploration`, `Dashboard API Routes 2`, `PaymentStatusUpdate`, `Agent Orchestrator`, `Dashboard ORM Models`, `Phase 5 — Escalations + staff chat`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `get_supabase_client()` connect `Drive Tooling` to `Chat Message Pipeline 4`, `Decision Graph 3`, `RAG Ingest Pipeline 2`, `Escalation Routes`, `seed_langfuse_prompts.py`, `Memory MCP Path`, `Decision Graph 2`, `Dashboard Module`, `kb_documents.py`, `test_identity_recall.py`, `Langfuse Runtime 2`, `Test Suite`, `Workshop Exploration`, `Admissions Onboarding 7`, `Phase 5 — Escalations + staff chat`, `tenant_scope.py`, `Escalation Routes 8`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `OnboardingFlow` connect `Admissions Onboarding` to `Admissions Onboarding 2`, `Test Suite 5`, `Workshop Exploration`?**
